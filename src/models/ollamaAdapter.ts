...
export class OllamaAdapter extends BaseOpenAIAdapter implements ModelAdapter {
  ...
  async isAvailable(): Promise<boolean> {
    try {
      ...
      logInfo('OllamaAdapter', `Ollama availability check: ${available ? 'available' : 'unavailable'}`);
      return available;
    } catch (error) {
      logWarn('OllamaAdapter', 'Ollama is not available', error);
      return false;
    }
  }
  ...
  override async getModels(): Promise<ModelInfo[]> {
    try {
      ...
      const ollamaModels = response.data.models as Array<{
        name: string;
        model: string;
        modified_at: string;
        size: number;
      ...
      logError('OllamaAdapter', 'Failed to get Ollama models', error);
      return [];
    }
  }
  ...
  async getModelCapabilities(model?: string): Promise<ModelCapabilities> {
    ...
    logDebug('OllamaAdapter', `Model capabilities for ${modelName}`, capabilities);
  ...
  async getContextWindow(model?: string): Promise<number> {
    ...
    try {
      ...
      logDebug('OllamaAdapter', 'Failed to get context window from Ollama model info', error);
    }
    ...
    logWarn('OllamaAdapter', `Using estimated context window: ${defaultWindow} for model ${modelName}`);
    
    return defaultWindow;
  }
  
  private async getOllamaModelInfo(modelName: string): Promise<OllamaModelInfo | null> {
    try {
      ...
      return response.data as OllamaModelInfo;
    } catch (error) {
      logDebug('OllamaAdapter', 'Failed to get model info from Ollama', error);
      return null;
    }
  }
  
  private detectContextWindowFromFamily(family: string, parameterSize: string): number | null {
