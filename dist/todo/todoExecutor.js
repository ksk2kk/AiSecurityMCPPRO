...
exports.TodoExecutor = void 0;
exports.getTodoExecutor = getTodoExecutor;
...
class TodoExecutor {
    static instance;
    todoManager = todoManager_1.TodoManager.getInstance();
    ...
    async executeList(listId, callbacks, tools) {
        const config = (0, config_1.getConfig)();
        const list = this.todoManager.getTodoList(listId);
        if (!list) {
            (0, logger_1.logError)('TodoExecutor', `Todo list not found: ${listId}`);
            return false;
        }
        if (this.isRunning) {
            (0, logger_1.logWarn)('TodoExecutor', 'Executor is already running');
            return false;
        }
        this.isRunning = true;
        this.shouldStop = false;
        (0, logger_1.logInfo)('TodoExecutor', `Starting execution of list: ${list.title}`);
        try {
            ...
            while (!this.shouldStop) {
                ...
                (0, logger_1.logInfo)('TodoExecutor', `Executing task: ${nextItem.content}`);
                let retryCount = 0;
                ...
                while (!success && retryCount <= maxRetries && !this.shouldStop) {
                    try {
                        ...
                        this.todoManager.updateItemStatus(listId, nextItem.id, 'completed', result.result, undefined);
                        ...
                        if (retryCount > maxRetries) {
                            (0, logger_1.logError)('TodoExecutor', `Task failed after ${maxRetries} retries: ${nextItem.content}`, error);
                            this.todoManager.updateItemStatus(listId, nextItem.id, 'failed', undefined, error instanceof Error ? error.message : String(error));
                            if (callbacks?.onItemFail) {
                                callbacks.onItemFail(nextItem, error instanceof Error ? error : new Error(String(error)));
                            }
                        }
                        else {
                            (0, logger_1.logWarn)('TodoExecutor', `Task failed, retry ${retryCount}/${maxRetries}: ${nextItem.content}`);
            ...
            (0, logger_1.logInfo)('TodoExecutor', `Execution complete for list: ${list.title}`);
            return true;
        }
        finally {
    ...
    async executeWithTool(item, tools) {
        const tool = tools.find(t => t.function.name === item.tool);
        if (!tool) {
            throw new Error(`Tool not found: ${item.tool}`);
        }
        (0, logger_1.logInfo)('TodoExecutor', `Executing with tool: ${item.tool}`);
...
You have access to tools to help complete this task. Use the appropriate tool to execute the task.`
        ...
        messages.push({
            role: 'user',
            content: `Execute the task: ${item.content}${item.toolArguments ? `\nArguments: ${JSON.stringify(item.toolArguments)}` : ''}`
        });
    ...
    async executeWithModel(item, tools) {
        (0, logger_1.logInfo)('TodoExecutor', `Executing with model interpretation: ${item.content}`);
        const messages = [
            {
                role: 'system',
...
Be practical and specific. If you need clarification, ask specific questions.`
    ...
    stop() {
        ...
        (0, logger_1.logInfo)('TodoExecutor', 'Stopping execution...');
    }
    isExecuting() {
...
exports.TodoExecutor = TodoExecutor;
function getTodoExecutor() {
    return TodoExecutor.getInstance();
}
//# sourceMappingURL=todoExecutor.js.map