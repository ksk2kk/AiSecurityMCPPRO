...
class ToolExecutor {
    ...
    executionHistory = [];
    constructor() {
        this.toolRegistry = (0, toolRegistry_1.getToolRegistry)();
    }
    static getInstance() {
    ...
    async checkToolInstalled(tool) {
        (0, logger_1.logDebug)('ToolExecutor', `Checking if ${tool.name} is installed...`);
        try {
            ...
            (0, logger_1.logInfo)('ToolExecutor', `${tool.name} check: installed=${installed}${version ? `, version=${version}` : ''}`);
            return { installed, version };
        }
        catch (error) {
            (0, logger_1.logDebug)('ToolExecutor', `${tool.name} not installed: ${error}`);
    ...
    async installTool(tool, onProgress) {
        ...
        if (!config.toolChain.autoInstall) {
            (0, logger_1.logWarn)('ToolExecutor', 'Auto-install is disabled in config');
            return false;
        }
        (0, logger_1.logInfo)('ToolExecutor', `Installing ${tool.name}...`);
        if (onProgress) {
            onProgress({
                toolName: tool.name,
                phase: 'checking',
        ...
        if (!tool.installCommand) {
            (0, logger_1.logError)('ToolExecutor', `${tool.name} has no install command`);
            if (onProgress) {
                onProgress({
                    toolName: tool.name,
                    phase: 'failed',
        ...
        try {
            ...
            (0, logger_1.logInfo)('ToolExecutor', `Successfully installed ${tool.name}`);
            return true;
        }
        catch (error) {
            (0, logger_1.logError)('ToolExecutor', `Failed to install ${tool.name}`, error);
            if (onProgress) {
                onProgress({
                    toolName: tool.name,
                    phase: 'failed',
    ...
    async updateTool(tool, onProgress) {
        ...
        if (!config.toolChain.autoUpdate) {
            (0, logger_1.logWarn)('ToolExecutor', 'Auto-update is disabled in config');
            return false;
        }
        (0, logger_1.logInfo)('ToolExecutor', `Updating ${tool.name}...`);
        // For now, re-run install as update
        return this.installTool(tool, onProgress);
    }
    async installAllTools(onProgress) {
        ...
        for (let i = 0; i < tools.length; i++) {
            ...
            try {
                ...
                (0, logger_1.logError)('ToolExecutor', `Error installing ${tool.name}`, error);
                failed++;
            }
        }
        (0, logger_1.logInfo)('ToolExecutor', `Installed ${installed}/${tools.length} tools (${failed} failed)`);
    ...
    async executeTool(toolName, args, options) {
        ...
        (0, logger_1.logInfo)('ToolExecutor', `Executing ${toolName}: ${command}`);
        ...
        try {
            ...
            (0, logger_1.logInfo)('ToolExecutor', `${toolName} execution ${execution.status} (exit code: ${result.exitCode})`);
            // Move to history
            ...
            (0, logger_1.logError)('ToolExecutor', `${toolName} execution failed`, error);
            this.activeExecutions.delete(execution.id);
            this.executionHistory.push(execution);
            return execution;
        }
    }
    buildCommand(tool, args) {
    ...
    formatArgument(name, value, type) {
        if (type === 'boolean') {
    ...
    async executeCommand(command, options) {
        return new Promise((resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                timedOut = true;
                proc.kill('SIGTERM');
                (0, logger_1.logWarn)('ToolExecutor', `Command timed out after ${options.timeout}ms: ${command}`);
            }, options.timeout);
    ...
    cancelExecution(id) {
        ...
        this.executionHistory.push(execution);
        (0, logger_1.logInfo)('ToolExecutor', `Cancelled execution: ${id}`);
        return true;
    }
}
