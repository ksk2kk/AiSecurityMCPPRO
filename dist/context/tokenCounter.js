...
class TokenCounter {
    ...
    estimateMaxMessagesForTokens(messages, maxTokens, reserveTokens = 0) {
        ...
        if (systemTokens > availableTokens) {
            (0, logger_1.logInfo)('TokenCounter', 'System messages exceed available tokens, returning only system messages');
            return systemMessages;
        }
        const remainingTokens = availableTokens - systemTokens;
        // Work backwards from the end for non-system messages
        const nonSystemMessages = messages.filter(m => m.role !== 'system');
        const selectedNonSystem = [];
        let currentTokens = 0;
        ...
        (0, logger_1.logInfo)('TokenCounter', `Selected ${result.length} messages (${systemMessages.length} system, ${selectedNonSystem.length} user/assistant) using ~${systemTokens + currentTokens} tokens`);
        return result;
    }
    truncateMessageToTokens(message, maxTokens) {
        if (!message.content) {
            return null;
        }
        ...
        (0, logger_1.logDebug)('TokenCounter', `Truncated message from ${content.length} to ${truncatedContent.length} chars`);
        return {
            ...message,
